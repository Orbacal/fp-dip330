...
// Darbs nepabeigts laika trukuma del
// Netika realizeta vardu skaitlosana, tacu regex atbilst visiem testiem


object WordCount {
    fun phrase(str: String): Map<String, Number> {
        val regex = ("""([\w'-]+)""").toRegex(RegexOption.IGNORE_CASE)
        val occur = regex.find(str)!!.value
        val theMap = mutableMapOf<String, Number>()
        theMap.put(occur,1)
        return theMap
    }
}
